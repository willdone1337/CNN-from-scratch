...
images, labels = (x_train[0:1000].reshape(1000, 28 * 28) / 255,
                  # transform train images to matrix 28*28 and devide to number of color in palitra
                  y_train[0:1000])
...
for j in range(iterations):
    ...
    for i in range(int(len(images) / batch_size)):
        ...
        / (batch_size * layer_2.shape[0])# difference between our predict and label
        ...
        weight_1_2 += alpha * layer_1.T.dot(
            layer_2_delta)  # gradient descent(multiply delta of layer to input and alpha for better angle)
